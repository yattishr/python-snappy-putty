...
def _resolve_in_scope(raw_path: str, cwd: Path) -> PathResolution:
...
def plan_mkdir(path: str, cwd: Path | None = None, op_id: str = "op1") -> PlannedOp:
    base = (cwd or Path.cwd()).resolve()
    resolved = _resolve_in_scope(path, base)
    if resolved.warning:
...
def _validate_file_source(src: str, cwd: Path) -> Path:
    resolved_src = _resolve_in_scope(src, cwd)
...
def _validate_destination(dst: str, cwd: Path, *, op_id: str, allow_missing_parent: bool = False) -> tuple[Path, PlannedOp | None]:
    resolved_dst = _resolve_in_scope(dst, cwd)
    if resolved_dst.warning:
        raise ValueError(resolved_dst.warning)
    assert resolved_dst.path is not None
    if resolved_dst.path.exists():
    ...
    return resolved_dst.path, mkdir_op
...
def plan_copy_file(
    ...
    *,
    ...
    base = (cwd or Path.cwd()).resolve()
    src_path = _validate_file_source(src, base)
    dst_path, _ = _validate_destination(dst, base, op_id=op_id, allow_missing_parent=allow_missing_parent)
    return PlannedOp(
        op_id=op_id,
        action="copy",
...
def plan_rename_file(
    ...
    *,
    ...
    dst_path, _ = _validate_destination(dst_hint, base, op_id=op_id, allow_missing_parent=allow_missing_parent)
    return PlannedOp(
        op_id=op_id,
        action="rename",
        src=_as_display_path(src_path, base),
...
def parse_incomplete_fs_intent(intent: str) -> tuple[str, str] | None:
    ...
    rest = [token for token in tokens[1:] if token.lower() not in FILLER_WORDS]
    if not rest:
        return None
    if "to" in [token.lower() for token in rest]:
        return None
    return action, rest[0]


def plan_fs_intent(intent: str, cwd: Path | None = None) -> FsPlan | None:
    base = (cwd or Path.cwd()).resolve()
    ...
    try:
        if "copy " in lower:
            ...
            if not parsed:
                return None
            src, dst = parsed
            resolved_dst = _resolve_in_scope(dst, base)
            if resolved_dst.warning:
            ...
            ops.append(plan_mkdir(path=dst, cwd=base, op_id="op1"))
        else:
            return None
    ...
    if len(ops) > MAX_OPS:
        warnings.append(f"Plan has {len(ops)} operations, which exceeds the maximum of {MAX_OPS}. Narrow the scope.")
        ops = []
...
def apply_fs_plan(plan: FsPlan, cwd: Path | None = None) -> FsApplyResult:
    ...
    for op in plan.ops:
        try:
            if op.action == "mkdir":
                ...
                target = _resolve_in_scope(op.dst, base)
                ...
                results.append(FsApplyItem(op_id=op.op_id, action=op.action, status="applied", message=f"Created directory: {op.dst}"))
            elif op.action == "copy":
                if op.src is None or op.dst is None:
                    raise ValueError("Missing source or destination for copy operation.")
                src = _resolve_in_scope(op.src, base)
                dst = _resolve_in_scope(op.dst, base)
                ...
                results.append(FsApplyItem(op_id=op.op_id, action=op.action, status="applied", message=f"Copied file: {op.src} -> {op.dst}"))
                ...
                results.append(FsApplyItem(op_id=op.op_id, action=op.action, status="applied", message=f"{verb}: {op.src} -> {op.dst}"))
            else:
